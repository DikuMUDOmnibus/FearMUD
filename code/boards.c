 ...
 * ------------------------------------------------------------------------ *
 *			     Special boards module (Old Ones)                         *     
 ****************************************************************************/
...
BOARD_DATA *find_board(CHAR_DATA * ch)
{
   ...
   {
      if ((board = get_board(obj)) != NULL && IN_SAME_ROOM_OBJ(ch, obj))
         return board;
   }
...
void write_board(BOARD_DATA * board)
{
   ...
   else
   {
      for (pnote = board->first_note; pnote; pnote = pnote->next)
      {
         fprintf(fp, "Sender  %s~\nDate    %s~\nTo      %s~\nSubject %s~\nVoting %d\nYesvotes %s~\nNovotes %s~\nAbstentions %s~\nText\n%s~\n\n",
            pnote->sender,
            pnote->date, pnote->to_list, pnote->subject, pnote->voting, pnote->yesvotes, pnote->novotes, pnote->abstentions, pnote->text);
      }
      fclose(fp);
   }
...
OBJ_DATA *find_inote(CHAR_DATA * ch)
{
   OBJ_DATA *note;

   for (note = ch->first_carrying; note; note = note->next_content)
   {
...
void do_note(CHAR_DATA * ch, char *arg_passed, bool IS_MAIL)
{
   ...
   int anum;
   int first_list;
   OBJ_DATA *quill = NULL, *paper = NULL;
   EXTRA_DESCR_DATA *ed = NULL;
   char notebuf[MSL];
   char short_desc_buf[MSL];
   char long_desc_buf[MSL];
   ...
   switch (ch->substate)
   {
      default:
         break;
      case SUB_WRITING_NOTE:
         if ((paper = find_inote(ch)) == NULL || paper->item_type != ITEM_PAPER)
         {
         ...
         for (pnote = board->first_note; pnote; pnote = pnote->next)
         {
            ...
            if ((first_list && vnum >= first_list) || !first_list)
               pager_printf(ch, "%2d%c %-12s%c %-24s : %-35s\n\r",
                  vnum,
                  is_note_to(ch, pnote) ? ')' : '}',
                  pnote->sender, (pnote->voting != VOTE_NONE) ? (pnote->voting == VOTE_OPEN ? 'V' : 'C') : ':', pnote->date, pnote->subject);
         }
   ...
   if (!str_cmp(arg, "read"))
   {
      ...
      if (!IS_MAIL)
      {
         ...
         for (pnote = board->first_note; pnote; pnote = pnote->next)
         {
            ...
            if (vnum == anum || fAll)
            {
               pager_printf(ch, "[%3d] %s: %s\n\r%s\n\rTo: %s\n\r%s", vnum, pnote->sender, pnote->subject, pnote->date, pnote->to_list, pnote->text);

               if (pnote->yesvotes[0] != '\0' || pnote->novotes[0] != '\0' || pnote->abstentions[0] != '\0')
               {
                  send_to_pager("------------------------------------------------------------\n\r", ch);
                  pager_printf(ch, "Votes:\n\rYes:     %s\n\rNo:      %s\n\rAbstain: %s\n\r", pnote->yesvotes, pnote->novotes, pnote->abstentions);
               }
...
/*	if (imc)
   ...
   if (!str_cmp(arg, "remove") || !str_cmp(arg, "take") || !str_cmp(arg, "copy"))
   {
      ...
      {
         send_to_char("Note remove which nunumber?\n\r", ch);
         return;
      }

      if (!can_read(ch, board))
      {
         send_to_char("You can't make anyense of what's posted here, let alone remove anything!\n\r", ch);
         return;
      }
...
void do_bstat(CHAR_DATA * ch, char *argument)
{
   ...
   ch_printf_color(ch,
      ...
      board->note_file, board->board_obj, board->min_read_level, board->min_post_level, board->min_remove_level, board->max_posts, board->type,
      board->num_posts);
...
void do_boards(CHAR_DATA * ch, char *argument)
{
   ...
   for (board = first_board; board; board = board->next)
      pager_printf_color(ch, "&G%-15.15s #: %5d Read: %2d Post: %2d Rmv: %2d Max: %3d Posts: &g%3d &GType: %d\n\r",
         ...
         board->min_read_level, board->min_post_level, board->min_remove_level, board->max_posts, board->num_posts, board->type);
}
