...
void do_setdeity(CHAR_DATA * ch, char *argument)
{
   ...
   if (arg1[0] == '\0')
   {
      ...
      send_to_char("race race2 npcrace element sex affected suscept elementnum affectednum\n\r", ch);
      send_to_char("\n\rFavor adjustments:\n\r", ch);
...
void do_showdeity(CHAR_DATA * ch, char *argument)
{
   ...
   ch_printf(ch, "Alignment: %-7dNpcrace: %-9sNpcfoe: %s\n\r",
      deity->alignment,
      (deity->npcrace < 0 || deity->npcrace > max_npc_race) ? "none" : print_npc_race(deity->npcrace),
      (deity->npcfoe < 0 || deity->npcfoe > max_npc_race) ? "none" : print_npc_race(deity->npcfoe));
   ch_printf(ch, "Race: %-12sSex: %-13sRace2: %s\n\r",
      (deity->race < 0 || deity->race > MAX_RACE) ? "none" : print_npc_race(deity->race),
      deity->sex == -1 ? "none" :
      deity->sex == SEX_MALE ? "male" :
      deity->sex == SEX_FEMALE ? "female" : "neutral", (deity->race2 < 0 || deity->race2 > MAX_RACE) ? "none" : print_npc_race(deity->race2));
...
void do_supplicate(CHAR_DATA * ch, char *argument)
{
   ...
   if (!str_cmp(arg, "corpse"))
   {
      ...
      {
         send_to_char("You cannot supplicate while on the Wilderness map.\n\r", ch);
         return;
      }
      ...
      for (obj = first_object; obj; obj = obj->next)
      {
         if (obj->in_room && !str_cmp(buf2, obj->short_descr) && (obj->pIndexData->vnum == 11))
         {
            ...
            {
               act(AT_MAGIC, "Your supplicate for your corpse, but it is protected in a PK zone.", ch, NULL, NULL, TO_CHAR);
               char_from_room(ch);
...
ch->pcdata->favor <= ch->pcdata->deity->elementnum) || (oldfavor < ch->pcdata->deity->susceptnum && ch->pcdata->favor >= ch->pcdata->deity->susceptnum))
   ...
   if (!str_cmp(arg, "recall"))
   {
      ...
      if (check_room_pk(ch) >= 3)
      {
         send_to_char("Your deity shuns cowards you KNOW!\n\r", ch);
         return;
      }

      if (in_hellmaze(ch))
      {
         send_to_char("Even your deity cannot hear you in this forsaken place.\n\r", ch);
         return;
      }
