      ...
      case SUB_NONE:
         {
            ...
            {
               level = atoi(arg2);
               if (level < 0 || level > LEVEL_ADMIN) /* Tracker1 */
               {
                  ch_printf(ch, "Level range is from 0 to %d.\n\r", LEVEL_ADMIN); /* Tracker1 */
                  return 0;
               }
            }
            else if (!str_cmp(arg2, "all"))
               level = LEVEL_ADMIN; /* Tracker1 */
         ...
         for (bnum = 1; pban; pban = pban->next, bnum++)
         {
            if (!pban->user)
               ...
               pager_printf(ch, "[%2d] %-4s (%2d) %-24s %-15s %4d  %s@%c%s%c\n\r",
                  bnum, (pban->warn) ? "YES" : "no", pban->level,
                  pban->ban_time, pban->ban_by, pban->duration, pban->user, (pban->prefix) ? '*' : ' ', pban->name, (pban->suffix) ? '*' : ' ');
         }
         return;
      case BAN_RACE:
...
bool check_total_bans(DESCRIPTOR_DATA * d)
{
   ...
   for (pban = first_ban; pban; pban = pban->next)
   {
      if (pban->level != LEVEL_ADMIN) /* Tracker1 */
         continue;
...
bool check_bans(DESCRIPTOR_DATA *d, CHAR_DATA * ch, int type, int nban)
{
   char buf[MSL];
   BAN_DATA *pban;
   char new_host[MSL];
   ...
   for (; pban; pban = pban->next)
   {
      ...
      if (type == BAN_SITE)
      {
         if (pban->prefix && pban->suffix && strstr(pban->name, new_host))
            ...
            fMatch = TRUE;
         if (fMatch && pban->user && str_cmp(pban->user, d->user))
            fMatch = FALSE;
         if (fMatch)
         {
            if (check_expire(pban))
            ...
            if (pban->level == 1 || pban->level == BAN_WARN)
            {
               if (nban == 1)
                  return TRUE;
               if (pban->warn)
               {
                  sprintf(buf, "%s logging in from site %s.", d->account->name, ch->desc->host);
                  log_string_plus(buf, LOG_WARN, sysdata.log_level);
               }
               return FALSE;
            }
