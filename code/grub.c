   ...
   if (tot_match > 0 && dis_num > 0) /* print title if app  */
   {
      if (!d2_sw)
         pager_printf(ch,
            ...
            "%11s\n\r", "Character", "Cou", "OVnum", "Lv", "OName", "Ty", "We", "Av", "Hr", "Dr", "Hp", "Mp", "AC", "S D C W I L");
      else
         pager_printf(ch,
            "\n\r%-12s%3s %5s %2s %-12s %2s %2s %2s %2s %2s %3s %3s %2s "
            "%2s %2s %2s %2s\n\r",
            "Character", "Cou", "OVnum", "Lv", "OName", "Ty", "We", "Av", "Hr", "Dr", "Hp", "Mp", "S0", "S1", "S2", "S3", "S4");
   }
   ...
   for (cou = 0; cou < lim; cou++)
   {
      ...
      if (!d2_sw)
         pager_printf(ch,
            ...
            r.n[OAVG], r.n[OHR], r.n[ODR], r.n[OHP], r.n[OMP], r.n[OAC], r.n[OSTR], r.n[ODEX], r.n[OCON], r.n[OWIS], r.n[OINT], r.n[OLUCK]);
      else
         pager_printf(ch,
            ...
            pri_cname, r.n[OCOUNT], r.n[OVNUM], r.n[OLEVEL],
            pri_oname, go_otype_to_disp(r.n[OTYPE]),
            owear_to_disp(r.n[OWEAR]), r.n[OAVG], r.n[OHR], r.n[ODR], r.n[OHP], r.n[OMP], r.n[OSAV0], r.n[OSAV1], r.n[OSAV2], r.n[OSAV3], r.n[OSAV4]);
   }
   if (tot_match == 0)
...
bool go_read(CHAR_DATA * ch, int dis_num, int op_num, int sor_ind, bool sor_dir, bool or_sw, bool np_sw, bool nm_sw, bool ng_sw, bool d2_sw)
{
   enum
   { OCOUNT, OVNUM, OTYPE, OLEVEL, OWEAR, OAVG, OHR, ODR, OHP, OMP, OAC,
      OSTR, ODEX, OCON, OWIS, OINT, OLUCK,
      ...
      if (res) /* record is a match         */
      {
         ...
         if (tot_match <= dis_num) /* print record if applicable */
            ch_printf(ch,
               ...
               clan[(unsigned char) r.clan], council[(unsigned char) r.council], r.room, r.gold, r.site, r.last, r.pkill);
      }
      fread(&r, sizeof(r), 1, fp);
   }
   fclose(fp);
      ...
      for (cou = 0; cou < MAX_KEY_HASH; cou++)
      {
         if (room_index_hash[cou])
            for (pRoom = room_index_hash[cou]; pRoom; pRoom = pRoom->next)
            {
               if (pRoom->vnum >= lo && pRoom->vnum <= hi)
               {
                  /*( match == (match & pRoom->room_flags) --Was in the if statement, don't know what to do with it Xer */
                  if (hit_cou < DIAG_RF_MAX_SIZE)
                     vnum[hit_cou++] = pRoom->vnum;
               }
            }
      }
...
#define ZERO_MAX   1500
   ...
   if (!str_cmp(arg1, "ob"))
   {
      ...
      ch_printf(ch, "CHAR DATA " "name=%s affected_by=%d perm_str=%d mod_str=%d\n\r", ch->name, ch->affected_by, ch->perm_str, ch->mod_str);

      for (pa = ch->first_affect; pa; pa = pa->next)
         ch_printf(ch,
            "   type=%d duration=%d location=%d modifier=%d bitvector=%d\n\r", pa->type, pa->duration, pa->location, pa->modifier, pa->bitvector);
...
*/
      ...
      for (cou = 0; cou < MAX_KEY_HASH; cou++)
      {
         if (mob_index_hash[cou])
            ...
            {
               if (pm->vnum >= vnum1 && pm->vnum <= vnum2 && pm->race == race && dis_cou++ < dis_num)
         ...
         for (pObj = obj_index_hash[hash]; pObj; pObj = pObj->next)
            if (pObj->layers > 0)
            {
               if (++cou <= display_limit)
                  pager_printf(ch, "%4d %5d %9d %5d   %s\n\r", cou, pObj->vnum, pObj->wear_flags, pObj->layers, pObj->short_descr);
            }
}
