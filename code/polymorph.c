...
void do_morphset(CHAR_DATA * ch, char *argument)
{
   ...
   if (arg1[0] == '\0' || (arg2[0] == '\0' && ch->substate != SUB_REPEATCMD) || !str_cmp(arg1, "?"))
   {
      ...
      send_to_char("  ac, affected, blood, bloodused, cha, con, damroll, dayto,\n\r", ch);
   ...
   else if (!str_cmp(arg2, "level"))
   {
      if (value < 0 || value > 2)
      {
         send_to_char("Level range is 0 to 1.\n\r", ch);
         return;
      }
      morph->level = value;
   }
...
void do_morphstat(CHAR_DATA * ch, char *argument)
{
   ...
   else if (!str_cmp(argument, "help") || !str_cmp(argument, "desc"))
   {
      ...
      pager_printf_color(ch, "  &cShortdesc   : &w%s\n\r", (morph->short_desc[0] == '\0') ? "(none set)" : morph->short_desc);
      pager_printf_color(ch, "  &cLongdesc    : &w%s", (morph->long_desc[0] == '\0') ? "(none set)\n\r" : morph->long_desc);
...
bool can_morph(CHAR_DATA * ch, MORPH_DATA * morph, bool is_cast)
{
   ...
   if (morph->deity && (!ch->pcdata->deity || !get_deity(morph->deity)))
      return FALSE;
   if (morph->timeto != -1 && morph->timefrom != -1 && (morph->timeto < gethour() || morph->timefrom > gethour()))
      return FALSE;
   if (morph->dayfrom != -1 && morph->dayto != -1 && (morph->dayto < (getday() + 1) || morph->dayfrom > (getday() + 1)))
      return FALSE;
   return TRUE;
}
...
void do_morph(CHAR_DATA * ch, MORPH_DATA * morph)
{
   ...
   ch_morph->hitroll = dice_parse(ch, morph->level, morph->hitroll, 1);
   ch->hitroll += ch_morph->hitroll;
   ch_morph->damroll = dice_parse(ch, morph->level, morph->damroll, 1);
   ch->damroll += ch_morph->damroll;
   ch_morph->hit = dice_parse(ch, morph->level, morph->hit, 1);
   if ((ch->hit + ch_morph->hit) > MAX_HPMANA)
      ch_morph->hit = (MAX_HPMANA - ch->hit);
   ch->hit += ch_morph->hit;
   ch_morph->move = dice_parse(ch, morph->level, morph->move, 1);
   if ((ch->move + ch_morph->move) > 32700)
      ch_morph->move = (32700 - ch->move);
   ch->move += ch_morph->move;
   if (!IS_NPC(ch) && IS_VAMPIRE(ch))
   {
      ch_morph->blood = dice_parse(ch, morph->level, morph->blood, 1);
      ch->pcdata->condition[COND_BLOODTHIRST] += ch_morph->blood;
   }
   else
   {
      ch_morph->mana = dice_parse(ch, morph->level, morph->mana, 1);
      if ((ch->mana + ch_morph->mana) > MAX_HPMANA)
         ch_morph->mana = (MAX_HPMANA - ch->mana);
