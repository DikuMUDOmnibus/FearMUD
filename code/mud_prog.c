...
//#define MAX_PROG_NEST   20      Moved to mud.h for mpsleep
...
int mprog_do_ifcheck(char *ifcheck, CHAR_DATA * mob, CHAR_DATA * actor, OBJ_DATA * obj, void *vo, CHAR_DATA * rndm)
{
   ...
   *pchck = '\0'; /* Gorog's bug fix */
   ...
   if (!str_cmp(chck, "minutes"))
   {
      ...
      tplayed = (actor->played + (current_time - actor->pcdata->logon)) / 60;
      lvalue = atoi(rval);
      if (!str_cmp(opr, "=="))
      {
         if (tplayed == lvalue)
      ...
      {
         progbug("minutes: bad check", mob);
         return BERR;
      }
      return FALSE;
   }
   if (!str_cmp(chck, "mastery"))
   {
      int sn = skill_lookup(cvar);
      int lvalue;
      int ranking;

      progbug("first check", mob);

      if (sn == -1)
      {
         progbug("mastery: bad skill name", mob);
         return BERR;
      }
      if (IS_NPC(actor))
      {
      ...
      lvalue = atoi(rval);
      ranking = actor->pcdata->ranking[sn];
      if (!str_cmp(opr, "=="))
      {
         if (ranking == lvalue)
            return TRUE;
...
/* Changed below from 1 to 0 so can check for == no items Shaddai */
   ...
   if (!str_cmp(chck, "ovnumcontainer"))
   {
      OBJ_DATA *sobj;
      OBJ_DATA *pObj;
      int vnum = atoi(cvar);
      ...
      if (vnum < 1 || vnum > MAX_VNUM)
      {
         progbug("OvnumContainer: bad vnum", mob);
         return BERR;
      }
      if (fvnum < 1 || fvnum > MAX_VNUM)
      {
         progbug("OvnumContainer: bad vnum 2", mob);
         return BERR;
      }
      if (!str_cmp(opr, "!="))
         optype = 1;
      ...
      for (sobj = pObj->first_content; sobj; sobj = sobj->next_content)
      {
         if (sobj->pIndexData->vnum == fvnum)
         {
            if (optype == 0)
               return TRUE;
            else
...
/* Changed below from 1 to 0 for == 0 Shaddai */
   ...
   if (chkchar)
   {
      ...
      if (!str_cmp(chck, "checkmpvalue"))
      {
         return mprog_veval(chkchar->tcount, opr, atoi(rval), mob);
      }
      if (!str_cmp(chck, "weight"))
      {
         return mprog_veval(get_ch_carry_weight(chkchar), opr, atoi(rval), mob);
      }
      if(!str_cmp(chck, "fame"))
      {
	 return mprog_veval(chkchar->fame, opr, atoi(rval), mob);
      }
      ...
      {
         if (IS_NPC(chkchar))
            return mprog_seval(print_npc_race(chkchar->race), opr, rval, mob);
      ...
      if (!str_cmp(chck, "caste"))
      {
         return mprog_veval(chkchar->pcdata->caste, opr, atoi(rval), mob);
      }
   ...
   {
      return mprog_veval(gethour(), opr, atoi(rval), mob);
   }
...
void mprog_driver(char *com_list, CHAR_DATA * mob, CHAR_DATA * actor, OBJ_DATA * obj, void *vo, bool single_step)
{
   ...
   if (mob == actor)
   {
      sprintf(buf, "Triggering oneself: mob %d", mob->pIndexData->vnum);
      log_string_plus(buf, LOG_BUILD, lvl);
      return;
   }

   if (++prog_nest > MAX_PROG_NEST)
   {
      sprintf(buf, "max_prog_next exceeded: mob %d", mob->pIndexData->vnum);
      log_string_plus(buf, LOG_BUILD, lvl);
      --prog_nest;
      return;
   }
   oldtime = mob->fight_timer;
   mob->fight_timer = 0;
   ...
   while (TRUE)
   {
      mob->fight_timer = 0; //Makes sure to reset the bloody timer
      ...
      if (cmnd[0] == '\0')
      {
         ...
         {
            sprintf(buf, "Missing endif: mob %d", mob->pIndexData->vnum);
            log_string_plus(buf, LOG_BUILD, lvl);
         }
         --prog_nest;
         mob->fight_timer = oldtime; //so they can execute their commands
         return;
      }
      ...
      switch (result)
      {
         ...
         case IFTRUE:
            ...
            if (iflevel == MAX_IFS)
            {
               sprintf(buf, "Maximum nested ifs exceeded: mob %d", mob->pIndexData->vnum);
               log_string_plus(buf, LOG_BUILD, lvl);
               --prog_nest;
               mob->fight_timer = oldtime; //so they can execute their commands
               return;
         ...
         case ORTRUE:
            ...
            {
               sprintf(buf, "Unmatched or: mob %d", mob->pIndexData->vnum);
               log_string_plus(buf, LOG_BUILD, lvl);
               --prog_nest;
               mob->fight_timer = oldtime; //so they can execute their commands
               return;
         ...
         case FOUNDELSE:
            ...
            {
               sprintf(buf, "Found else in an else section: mob %d", mob->pIndexData->vnum);
               log_string_plus(buf, LOG_BUILD, lvl);
               --prog_nest;
               mob->fight_timer = oldtime; //so they can execute their commands
               return;
            ...
            if (!ifstate[iflevel][IN_IF])
            {
               sprintf(buf, "Unmatched else: mob %d", mob->pIndexData->vnum);
               log_string_plus(buf, LOG_BUILD, lvl);
               --prog_nest;
               mob->fight_timer = oldtime; //so they can execute their commands
               return;
         ...
         case FOUNDENDIF:
            ...
            if (!(ifstate[iflevel][IN_IF] || ifstate[iflevel][IN_ELSE]))
            {
               sprintf(buf, "Unmatched endif: mob %d", mob->pIndexData->vnum);
               log_string_plus(buf, LOG_BUILD, lvl);
               --prog_nest;
               mob->fight_timer = oldtime; //so they can execute their commands
               return;
         ...
         case IFIGNORED:
            ...
            if (!(ifstate[iflevel][IN_IF] || ifstate[iflevel][IN_ELSE]))
            {
               sprintf(buf, "Parse error, ignoring if while not in if or else: mob %d", mob->pIndexData->vnum);
               log_string_plus(buf, LOG_BUILD, lvl);
               --prog_nest;
               return;
            }
         ...
         case ORIGNORED:
            ...
            if (ignorelevel == 0)
            {
               sprintf(buf, "Parse error, mistakenly ignoring or: mob %d", mob->pIndexData->vnum);
               log_string_plus(buf, LOG_BUILD, lvl);
               --prog_nest;
               mob->fight_timer = oldtime; //so they can execute their commands
               return;
...
void mprog_time_check(CHAR_DATA * mob, CHAR_DATA * actor, OBJ_DATA * obj, void *vo, int type)
{
   ...
   for (mprg = mob->pIndexData->mudprogs; mprg; mprg = mprg->next)
   {
      trigger_time = (gethour() == atoi(mprg->arglist));
...
void mprog_speech_trigger(char *txt, CHAR_DATA * actor)
{
   ...
   {
      if (actor->coord->x > -1 || actor->coord->y > -1 || actor->map > -1 || vmob->coord->x > -1 || vmob->coord->x > -1 || vmob->map > -1)
      {
         if (actor->coord->x != vmob->coord->x || actor->coord->y != vmob->coord->y || actor->map != vmob->map)
            continue;
      }
...
void mprog_script_trigger(CHAR_DATA * mob)
{
   ...
   if (HAS_PROG(mob->pIndexData, SCRIPT_PROG))
      for (mprg = mob->pIndexData->mudprogs; mprg; mprg = mprg->next)
         if (mprg->type == SCRIPT_PROG && (mprg->arglist[0] == '\0' || mob->mpscriptpos != 0 || atoi(mprg->arglist) == gethour()))
...
void oprog_script_trigger(OBJ_DATA * obj)
{
   ...
   if (HAS_PROG(obj->pIndexData, SCRIPT_PROG))
      for (mprg = obj->pIndexData->mudprogs; mprg; mprg = mprg->next)
         ...
         {
            if (mprg->arglist[0] == '\0' || obj->mpscriptpos != 0 || atoi(mprg->arglist) == gethour())
            {
               set_supermob(obj);
...
void rprog_script_trigger(ROOM_INDEX_DATA * room)
{
   ...
   if (HAS_PROG(room, SCRIPT_PROG))
      for (mprg = room->mudprogs; mprg; mprg = mprg->next)
         if (mprg->type == SCRIPT_PROG)
         {
            if (mprg->arglist[0] == '\0' || room->mpscriptpos != 0 || atoi(mprg->arglist) == gethour())
            {
               rset_supermob(room);
               mprog_driver(mprg->comlist, supermob, NULL, NULL, NULL, TRUE);
...
void set_supermob(OBJ_DATA * obj)
{
   ...
   if (room != NULL)
   {
      ...
      {
         SET_ONMAP_FLAG(supermob);
         supermob->map = obj->map;
         supermob->coord->x = obj->coord->x;
         supermob->coord->y = obj->coord->y;
      }
   }
}
...
void release_supermob()
{
   ...
   if (IS_ONMAP_FLAG(supermob))
   {
      REMOVE_ONMAP_FLAG(supermob);
      supermob->map = -1;
      supermob->coord->x = -1;
      supermob->coord->y = -1;
   }
